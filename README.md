...
- 首次在项目目录运行时**自动创建 CHCODE.md**，跨会话积累项目知识（常用命令、包管理器、编码规范、禁止事项、验证流程、踩过的坑等）
- 目录下存在非空 `CLAUDE.md` 且 `CHCODE.md` 为空时**自动迁移**其内容（原文件保留），此后始终以 `CHCODE.md` 为准
- 每次会话以 `<system-reminder>` 元消息注入对话上下文；外部编辑在下一回合以增量 diff 提醒生效
- 通过 `update_memory` 专用工具保存记忆（Common 模式需确认），强制条目**简短、与行动相关、以约束表述**，杜绝接口文档/流水账/空泛口号
- **容量三层控制** — 超 8K 字符自动提醒 AI 清理过期条目；超 16K 截断注入并阻止新增（先删后加），文件本身永不静默修改
