...
class EnhancedChatOpenAI(ChatOpenAI):
    ...
    def _stream_response_to_generation_chunk(
        ...
        """Override to handle reasoning in streaming responses.

        Note: This method is not used in the standard _stream flow,
        but we keep it for compatibility with other potential callers.
        """
        chunk = super()._stream_response_to_generation_chunk(stream_response)

        # The reasoning extraction now happens in _convert_chunk_to_generation_chunk
        # This method is kept for backward compatibility
        return chunk

    def _message_to_dict(self, message: AIMessage) -> dict[str, Any]:
        """Convert message to dict, preserving reasoning."""
        message_dict = {
            "content": message.content,
            "role": "assistant",
        }

        # Include reasoning in additional_kwargs
        if message.additional_kwargs:
            message_dict["additional_kwargs"] = message.additional_kwargs.copy()
...
def create_qwen_chat(
    ...
    **kwargs: Any,
    ...
    """Create an EnhancedChatOpenAI instance configured for Qwen models.

    Args:
        model: Qwen model name (e.g., "qwen-plus", "qwen-max")
        api_key: DashScope API key
        base_url: Qwen API base URL
        **kwargs: Additional arguments passed to EnhancedChatOpenAI

    Returns:
        Configured EnhancedChatOpenAI instance for Qwen
    """
...
def create_deepseek_chat(
    ...
    **kwargs: Any,
    ...
    """Create an EnhancedChatOpenAI instance configured for DeepSeek models.
    ...
    Returns:
        Configured EnhancedChatOpenAI instance for DeepSeek
    """
    return EnhancedChatOpenAI(
        model=model,
        api_key=api_key,
...
def create_glm_chat(
    ...
    **kwargs: Any,
    ...
    """Create an EnhancedChatOpenAI instance configured for GLM models.

    Args:
        model: GLM model name (e.g., "glm-4", "glm-4-plus")
        api_key: Zhipu AI API key
        base_url: GLM API base URL
        **kwargs: Additional arguments passed to EnhancedChatOpenAI

    Returns:
        Configured EnhancedChatOpenAI instance for GLM
    """
    return EnhancedChatOpenAI(
        model=model,
        api_key=api_key,
...
def create_chat(
    provider: Literal["qwen", "deepseek", "glm", "moonshot"],
    model: str,
    api_key: str | None = None,
    **kwargs: Any,
) -> EnhancedChatOpenAI:
    """Create an EnhancedChatOpenAI instance for a specific provider.

    Args:
        provider: Model provider (qwen, deepseek, glm, moonshot)
        model: Model name
        api_key: API key for the provider
        **kwargs: Additional arguments

    Returns:
        Configured EnhancedChatOpenAI instance
    ...
    return EnhancedChatOpenAI(model=model, api_key=api_key, **config)
